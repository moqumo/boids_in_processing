...
class Boid {
  ...
  */
  ...
  /*
  パラメータを規定値に初期化するメソッド
  !!!: 値は変更しないこと。変更するときは、「params」タブのvoid setParams(Boidas)から
  */
  ...
  */
  public Boid fleeWall() {
    ...
    //x軸方向
    if (this.pos.x + this.vel.x + this.sensor_radius > width) sum_force_x = -this.default_speed/abs(width - this.pos.x);
    if (this.pos.x - this.vel.x - this.sensor_radius < 0)     sum_force_x = this.default_speed/abs( this.pos.x - 0);
    
    //y軸方向
    if (this.pos.y + this.vel.y + this.sensor_radius > height) sum_force_y = -this.default_speed/abs(height - this.pos.y);
    if (this.pos.y - this.vel.y - this.sensor_radius < 0)      sum_force_y = this.default_speed/abs( this.pos.y - 0);
  ...
  */
  public Boid death(){
    int flg = 0;
    //if(this.dist(new PVector(mouseX, mouseY)) < 20) flg++;
