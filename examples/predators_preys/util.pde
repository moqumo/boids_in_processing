...
枠外に出ることを判断
*/
PVector limitWall(PVector pos){
  PVector ret = pos;
  int revision = 50;
  int margin = 0;
  if(pos.x < 0 + revision)     ret.x = 0 + revision + margin;
  if(pos.x > width - revision) ret.x = width - revision - margin;
  if(pos.y < 0 + revision)     ret.y = 0 + revision + margin;
  if(pos.y > height - revision)ret.y = height - revision - margin;
  return ret;
}
